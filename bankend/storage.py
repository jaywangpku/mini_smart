...
class Storage:
    ...
    def get_custom_factor_by_code(self, code: str) -> dict | None:
        with self.connect() as conn:
            row = conn.execute("SELECT * FROM custom_factors WHERE code = ?", (code,)).fetchone()
            return dict(row) if row else None
    ...
    def list_custom_strategies(self, enabled_only: bool = False) -> list[dict]:
        clauses: list[str] = []
        if enabled_only:
            clauses.append("enabled = 1")
        with self.connect() as conn:
    ...
    def get_custom_strategy(self, strategy_id: str) -> dict | None:
        with self.connect() as conn:
            row = conn.execute("SELECT * FROM custom_strategies WHERE id = ?", (strategy_id,)).fetchone()
            return dict(row) if row else None

    def get_custom_strategy_by_code(self, code: str) -> dict | None:
        with self.connect() as conn:
            row = conn.execute("SELECT * FROM custom_strategies WHERE code = ?", (code,)).fetchone()
            return dict(row) if row else None
    ...
    def create_custom_strategy(
        ...
        with self.connect() as conn:
            conn.execute(
                ...
                """,
                (strategy_id, code, name, description, source_code, default_params, 1 if enabled else 0, now, now),
            )
        strategy = self.get_custom_strategy(strategy_id)
        if strategy is None:
    ...
    def update_custom_strategy(
        ...
        with self.connect() as conn:
            cursor = conn.execute(f"UPDATE custom_strategies SET {', '.join(fields)} WHERE id = ?", params)
        return self.get_custom_strategy(strategy_id) if cursor.rowcount else None

    def delete_custom_strategy(self, strategy_id: str) -> bool:
        with self.connect() as conn:
            cursor = conn.execute("DELETE FROM custom_strategies WHERE id = ?", (strategy_id,))
            return cursor.rowcount > 0
